...
export class DashboardComponent {
      ...
      next: (res) => {
        ...
        if (obj) {
          ...
          for (let item of obj) {
            ...
            this.numeroBeneficiarios.push(
              parseFloat(item.numero_beneficiarios)
            );
          }

          let topTres = this.getTopTresInstituciones(
            this.nombresInstituciones,
            this.numeroBeneficiarios
          );
  ...
  getTopTresInstituciones(
    ...
    let lista = nombresInstituciones.map((institucion, i) => ({
      institucion,
      beneficiarios: numeroBeneficiarios[i],
    }));
  ...
  getClasificacionInstituciones(
    ...
    let lista = nombresInstituciones.map((institucion, i) => {
      ...
      if (numeroBeneficiarios[i] >= 1 && numeroBeneficiarios[i] <= 300) {
        ...
        numeroBeneficiarios[i] >= 301 &&
        numeroBeneficiarios[i] <= 700
      ) {
        clasificacion = 'Plata';
    ...
    let mejorOro = lista.find((x) => x.clasificacion === 'Oro') || {
      institucion: '',
      beneficiarios: 0,
      clasificacion: '',
    };
    let mejorPlata = lista.find(
      (x) =>
        x.clasificacion === 'Plata' && x.institucion !== mejorOro.institucion
    ) || { institucion: '', beneficiarios: 0, clasificacion: '' };
    let mejorBronce = lista.find(
      (x) =>
        x.clasificacion === 'Bronce' &&
        x.institucion !== mejorOro.institucion &&
        x.institucion !== mejorPlata.institucion
    ) || { institucion: '', beneficiarios: 0, clasificacion: '' };

    // Crear nuevas listas para las instituciones, los números de beneficiarios y las clasificaciones
    this.InstitucionesClasificacion = [
      mejorOro.institucion,
      mejorPlata.institucion,
      mejorBronce.institucion,
    ];
    this.BeneficiariosClasificacion = [
      mejorOro.beneficiarios,
      mejorPlata.beneficiarios,
      mejorBronce.beneficiarios,
    ];
    this.ClasificacionInstituciones = [
      mejorOro.clasificacion,
      mejorPlata.clasificacion,
      mejorBronce.clasificacion,
    ];
    ...
    this.api.DataInstituciones().subscribe({
      next: (res) => {
        ...
        if (obj) {
          ...
          this.getClasificacionInstituciones(
            this.nombresInstituciones,
            this.numeroBeneficiarios
          );
          this.chartPolarAreaGraphic(
            this.topInstituciones,
            this.topBeneficiarios
          );
          this.chartBarGraphic(
            this.InstitucionesClasificacion,
            this.BeneficiariosClasificacion
          );
          ...
          for (let item of obj) {
            ...
            if (item.estado && item.estado[0] && item.estado[0].nombre_estado) {
              ...
              if (item.estado[0].nombre_estado.toLowerCase() == 'activa') {
                ...
                item.estado[0].nombre_estado.toLowerCase() == 'pasiva'
              ) {
                totalPasivas++;
              }
            } else {
  ...
  async getBeneficiariosXAnio() {
    try {
      ...
      const resp: any = await this.api.getBeneficiariosXAnio().subscribe({
        next: (data: any) => {
          let obj = data;
          if (obj) {
            // Asegúrate de que obj no es undefined
            ...
            this.lista_anios = this.lista_anios.concat(
              this.listaBeneficiarios[0]
            );
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text:
              'Mejores Instituciones Sociales Bronce, Plata y Oro ' +
              this.anioFinal,
            font: {
      ...
      options: {
        plugins: {
          ...
          title: {
            display: true,
            text:
              'Total De Instituciones Sociales Por Estado ' +
              this.anioFinal +
              ': ' +
              this.institucionesTotales,
            font: {
              size: 24,
            },
          },
  ...
  async obtenerChartPorAnios() {
    ...
    const resp: any = await this.api
      .getBeneficiariosRangoAnio(this.anioInicio, this.anioFinal)
      .subscribe({
        next: (data: any) => {
          // this.datosCargados = false;
          console.log(data);
  ...
  async obtenerPiePorInstituciones() {
    const resp: any = await this.api
      .getInstitucionesEstado(this.anioFinal)
      .subscribe({
        next: (data: any) => {
          let obj = data;
          if (obj) {
            ...
            if (obj.length > 0) {
              this.institucionesActivas =
                obj[0]['numero_instituciones_activas'];
              this.institucionesPasivas =
                obj[0]['numero_instituciones_pasivas'];
  ...
  obtenerBarPorCategoria() {
    ...
    const resp: any = this.api
      .getInstitucionesCategoria(this.anioFinal)
      .subscribe({
        next: (data: any) => {
          if (this.myChart) {
            this.myChart.destroy();
          ...
          for (let i of data) {
            const resp1 = this.api
              .DataInstitucionesId(i['id_institucion'])
              .subscribe({
                next: (data1: any) => {
                  labels_instituciones = labels_instituciones.concat(
                    data1.nombre
                  );
                  lista_instituciones = lista_instituciones.concat(
                    data1.numero_beneficiarios
                  );
                  console.log(
                    'data',
                    lista_instituciones,
